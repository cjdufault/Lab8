...
namespace GeoQuizTwo
{
    public partial class Form1 : Form
    {
        ...
        {
            string answer = questions.Values[questionCounter];

            FormCheat frmCheat = new FormCheat();
            frmCheat.Tag = answer; // pass the answer to the cheat form

            DialogResult cheatResult = frmCheat.ShowDialog();
            
            // "I'm Lazy" will be treated as if it's the OK button
            if (cheatResult == DialogResult.OK)
            {
                txtAnswer.Text = answer; // fill in the answer if user pressed "I'm Lazy"
            }
        }
    }
}
